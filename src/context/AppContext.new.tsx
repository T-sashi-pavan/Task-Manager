...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [dbState, dbOperations] = useDatabase();
  ...
  useEffect(() => {
    const loadUserData = async () => {
      if (state.user && !isInitialized) {
        try {
          const userData = await dbOperations.loadUserData(state.user.id);
          dispatch({ type: 'SET_TASKS', payload: userData.tasks });
          dispatch({ type: 'SET_TIME_SESSIONS', payload: userData.timeSessions });
          dispatch({ type: 'SET_DARK_MODE', payload: userData.settings.darkMode });
          dispatch({ type: 'SET_CURRENT_VIEW', payload: userData.settings.currentView as AppState['currentView'] });
    ...
    loadUserData();
  }, [state.user, dbOperations, isInitialized]);

  const register = async (userData: { username: string; email: string; password: string }) => {
    try {
      const user = await dbOperations.register(userData);
  ...
  const login = async (email: string, password: string) => {
    try {
      const user = await dbOperations.login(email, password);
      dispatch({ type: 'LOGIN', payload: user });
  ...
  const addTask = async (taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'timeSpent'>) => {
    ...
    try {
      const task = await dbOperations.createTask(taskData, state.user.id);
  ...
  const stopTimer = async () => {
    if (state.activeTimer && state.user) {
      ...
      try {
        const session = await dbOperations.createTimeSession(sessionData, state.user.id);
  ...
  const toggleDarkMode = async () => {
    ...
    try {
      await dbOperations.updateUserSettings(state.user.id, { darkMode: newDarkMode });
  ...
  const setCurrentView = async (view: AppState['currentView']) => {
    ...
    try {
      await dbOperations.updateUserSettings(state.user.id, { currentView: view });
